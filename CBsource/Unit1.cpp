...
unsigned int OpenSchedule(AnsiString in_dir, AnsiString in_SchedFile, vector <t_Event> & Base)
{
  ...
  if( FileExists(in_SchedFile) ) // new format shedule
  {
    ...
    reccount = MainForm->Memo2->Lines->Count;
    correct_event = false;  Founded_rec = 0;
    for(i=0;i<reccount;i++)
    {
      ...
      if(correct_event && (Founded_rec==1))
      {
        ...
        for(k=0;k<DEF_PARAMETERS_COUNT;k++)
        {
          ...
          if(TryStrToInt(ws,num))  { if(num<=0) num = 0; } else num = 0;
          switch (k)
          {
            case 0: { temp_list[0].start_date.year = num; break; }
            case 1: { temp_list[0].start_date.month = num; break; }
            case 2: { temp_list[0].start_date.day  = num; break; }
            case 3: { temp_list[0].start_date.weekday = num; break; }
            case 4: { temp_list[0].start_date.use_year = (num == 1); break; }
            case 5: { temp_list[0].start_date.use_month = (num == 1); break; }
            case 6: { temp_list[0].start_date.use_day = (num == 1); break; }
            case 7: { temp_list[0].start_date.use_weekly = (num == 1); break; }
            case 8: { temp_list[0].start_date.exclusive_day = (num == 1); break; }
            case 9: { temp_list[0].start_time.use_hour = (num == 1); break; }
            case 10: { temp_list[0].start_time.use_minute = (num == 1); break; }
            case 11: { temp_list[0].start_time.use_second = (num == 1); break; }
            case 12: { temp_list[0].start_time.hour = num; break; }
            case 13: { temp_list[0].start_time.minute = num; break; }
            case 14: { temp_list[0].start_time.second = num; break; }
            case 15: { temp_list[0].cycle_flags.all_flags = num; break; }
            case 16: { temp_list[0].event_sign.all_signs = num; break; }
            case 17: { temp_list[0].cycle_flags.el.id_rfile = num; break; }
          }//switch
        }//for k
      ...
      if(correct_event && (Founded_rec==2))  // sound file
      {
        ...
        {
          AddSoundList(ws);
          temp_list[0].event_sign.el.is_sound = 1;
        }
        else
        {
  ...
  else
  {
    ...
    for(i=0;i<100;i++)
    {
      ...
      if( FileExists(in_dir+ws) )
      {
        ...
        MainForm->Memo2->Lines->Clear();
        MainForm->Memo2->Lines->LoadFromFile(in_dir+ws);
    ...
    if( FileExists(in_dir+OLD_WEEK_FILE) )
    {
      res |= IS_WEEK_FILE;

      MainForm->Memo2->Lines->Clear();
      MainForm->Memo2->Lines->LoadFromFile(in_dir+OLD_WEEK_FILE);
    ...
    if( FileExists(in_dir+OLD_EX_FILE) )
    {
      res |= IS_EX_FILE;
      MainForm->Memo2->Lines->Clear();
      MainForm->Memo2->Lines->LoadFromFile(in_dir+OLD_EX_FILE);
            ...
            if(temp_list[MAX_OLD_EVENTS].cycle_flags.el.id_rfile != -1)
            {
              for(k=0;k<Founded_rec;k++)
              {
                if(temp_list[k].cycle_flags.el.id_rfile == temp_list[MAX_OLD_EVENTS].cycle_flags.el.id_rfile)
                {
                  ...
                  temp_list[MAX_OLD_EVENTS].cycle_flags.el.id_event = Base.size();
                  Base.push_back(temp_list[MAX_OLD_EVENTS]);
                  //temp_list[k].cycle_flags.el.id_rfile = -1;
    ...
    for(k=0;k<Founded_rec;k++)
    {
      if(temp_list[k].cycle_flags.el.id_rfile != -1)
      {
        ...
        MainForm->Memo1->Lines->Add("Size of Base "+IntToStr(Base.size()));
      }
    }// for k events

    SaveBase(in_SchedFile);
    MainForm->Memo1->Lines->Add("Size of BaseEvent "+IntToStr(Base.size()));
...
void ShowBase(vector <t_Event> & base)
{
 int i, CurRow, c_f, c_e;
 AnsiString str_date, str_time, str_buf;
  ...
  if( ! base.empty() )
    {
     i=base.size();
#ifdef GRID_TITLE
     MainForm->CurrentBase->RowCount = i+1;
     CurRow=1;
#else
     MainForm->CurrentBase->RowCount = i;
     CurRow=0;
#endif

     while ( iter != base.end() )
       {
        MomentToStr(iter->start_date, iter->start_time, &str_date, &str_time);
        if(str_date != "")
        {
...
void DoSignal(t_Event & base)
{
  ...
  MomentToStr(base.start_date, base.start_time, &str_day, &str_time);
  if(str_day != "")
  {
  ...
  if((base.event_sign.el.is_ring == 1) && IsCOMOpen())
  {
    CheckRingDelay = true;
    res = UpRing(base.event_sign.el.ring_time, &io_res);
  }
...
void __fastcall TMainForm::FormCreate(TObject *Sender)
{
  ...
  res = OpenSchedule(wdir, WorkScheduleFile, BaseEvent);

#ifdef GRID_TITLE
 // MainForm->CurrentBase->Cells[0][0] = "Момент";
...
void __fastcall TMainForm::Timer1Timer(TObject *Sender)
{
  ...
  else
  {
    ...
    {
      NowEvent = CheckSchedule(TodayList);
      if( NowEvent != -1)
      {
       Application->Restore();
